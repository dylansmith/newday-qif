...
describe('cli', () => {
  ...
  beforeEach(() => {
    ...
    // prevent writes
    cli.writeTargetFile = jest.fn(() => true)
  })
  ...
  describe('#withArgs', () => {
    ...
    it('should transform all csv files in a directory', async () => {
      ...
      expect(transformer.buildTargetPath).toHaveBeenNthCalledWith(1, `${fixturesDir}/fixture_input_empty.csv`)
      expect(transformer.buildTargetPath).toHaveBeenNthCalledWith(2, `${fixturesDir}/fixture_input.csv`)
      console.log(transformer.transformInput.mock)
